...
func _ready() -> void:
#	player_character.get_child(2).PlayerExitFinished.connect(on_player_anim_done())
...
func on_player_anim_done():
	pass

func on_planet_change_requested(planet_ID : int):
	var requested_planet = planet_nodes[planet_ID]
	
	#iterate through doors and set current_planet doors to ActiveDoors = true and else to ActiveDoors = false
